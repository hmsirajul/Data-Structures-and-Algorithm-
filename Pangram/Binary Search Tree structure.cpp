...
{
    if (root == NULL || root->data == key) // if key is found or if we have reached the end of tree, return that node
        return root;
    if (key < root->data) // if key is lesser than root, search the left subtree of root
        return searchNode(root->left, key);
    if (key > root->data) // is greater than root, search the right subtree of root
...
{
    while (root->right != NULL) // The rightmost value is the maximum value
        root = root->right;
    return root;
}
...
node deleteNode(node root, int key)
{
    ...
    else
    {
        ...
        node temp = minimumValue(root->right); // Replacing deleted node with minimum value of Right Subtree
        root->data = temp->data;
...
int main()
{
    ...
    cout << "\nsearchNode" << endl;
    cout<<searchNode(main_root,5)->data;
